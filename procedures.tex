...
\readsafetychapterfirst{}

This chapter is a reference guide and not to be used as a training manual.  New workers must be trained by experienced workers over at least two cooldowns before being allowed to perform these procedures without oversight.
...
TLs are evacuated with a leak detector (LD) until the leak rate plateaus.  It is recommended to pump overnight, but in a bind it can be done quicker.  An evacuated TL will be fine for months, but in practice it is safer to pump down each TL a few days before beginning a cooldown in case the vacuum space was somehow compromised in storage/transport.
...
\begin{enumerate}
 \item Plug in and turn on the LD to warm it up.
 ...
 \item Support the weight of both ends of the TL on stable objects and attach the KF adapter to the leak detector (we use a chair and lab bench in Figure \ref{fig:cf-adapter-pushedin}).  Do not allow the TL to torque about the adapter or the TL and adapter may both be damaged.
 ...
 \item Start the LD pump and wait for 2 minutes.  This should bring the leak rate (LR) to a low enough value to detect an external leak.  Spray both vacuum connections on the adapter with helium.
 ...
 \item Again, spray both joints with helium gas.  Start over and remake the leaking connection if necessary.
...
\begin{enumerate}
 ...
 \item Pull the stem back about an inch.  \textbf{If you pull too far you will break the adapter; see Figure \ref{fig:cf-adapter-pulledout}.}  If you hear a hiss, the vacuum line is now vented.  Start the procedure again at the beginning of Section \ref{cryofabTLs}.
...
\item leak detector (LD) with KF25 o-ring/clamp
\item ziplock bag
\end{itemize}
\end{minipage}
...
\begin{figure}
 ...
 \caption{The three ports on the LTL: A) emergency relief popoff, B) vacuum jacket pump port and brass placeholder screw, C) side screw valve.}
 \label{fig:ltl-ports}
\end{figure}
...
\textbf{Latex gloves} are for personnel safety and to keep from contaminating the indium.

A \textbf{variable alternating current} unit adjusts the temperature of the heating tape.

The indium is extuded onto sterile \textbf{\com{Absorbond Wiper Sheets, TX}} 9\inches x 9\inches, and kept for storage for later use.
...
\begin{figure}[htbp!]
 ...
 % indium-seal-block-mc.jpg: 0x0 pixel, 0dpi, nanxnan cm, bb=
...
\begin{figure}[htbp!]
 ...
 % indium-seal-mc-chemistry-clamp.jpg: 0x0 pixel, 0dpi, 0.00x0.00 cm, bb=
...
Do not attempt this procedure until first being cleared by DFELL mechanical operations (see Joe Faircloth).  All workers who operate the hoist must first be trained.
...
Hifrost only uses isopropyl alcohol for cleaning vacuum parts.  Other solvents, such as methanol, are used in the DFELL, but they are not to be used for Hifrost.

Most o-rings are for KF joints.  It is not practical to clean every o-ring every time it is touched, and this is unnecessary because our vacuum system is not UHV, so most KF joints can be visually inspected for dirt before use.
...
\begin{enumerate}
 ...
 \item replace the o-ring over the centerpiece
...
\begin{figure}[htbp!]
 ...
 % kenol-connectors-installation.jpg: 0x0 pixel, 0dpi, 0.00x0.00 cm, bb=
...
\begin{enumerate}
 ...
 \item While wearing cryogenic gloves, goggles and a face shield, slowly lower the level probe into the main flow valve. A smaller cold plume will exhaust upwards as the level probe cools down.
...
A leak detector is the proper way to determine if a vacuum volume is leak tight.  However, it is not always practical and sometimes very difficult to leak check a large volume, so we use a vacuum rise test as a course characterization of the vacuum fidelity.
...
Read Section \ref{making-indium-seals} on making indium seals.
...
Loosely turn in all the triple flange screws, careful not to strip the aluminum taps.  Tighten in a star formation so the o-ring does not become unevenly compressed.
